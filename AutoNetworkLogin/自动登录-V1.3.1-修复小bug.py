...
class NetworkLoginApp(QMainWindow):
    def __init__(self):
        ...
        self.periodic_login_interval = self.config['Settings'].get('periodic_login_interval', 0)  # 仅存储间隔值，不用于控制开关
        self.periodic_login_enabled = self.config['Settings'].get('periodic_login_enabled', False)  # 独立控制定期登录开关
        ...
        if self.forced_periodic_login:
            ...
            self.periodic_login_enabled = True
...
'''
    ...
    def save_config(self):
        """保存配置到文件"""
        # 确保保存开关状态
        self.config['Settings']['periodic_login_enabled'] = self.periodic_login_enabled
        with open(self.config_file, 'w', encoding='utf-8') as f:
    ...
    def update_status_display(self):
        ...
        if self.periodic_login_enabled:
            status_parts.append(f"<font color='green'>定时登录[开启]</font>")
        else:
    ...
    def toggle_periodic_login(self, checked=None):
        ...
        # 更新定期登录开关状态（独立存储，不修改间隔值）
        self.periodic_login_enabled = is_enabled
        
        # 如果开启且间隔为0，设置默认间隔（仅用于UI显示，不影响开关逻辑）
    ...
    def reload_config(self):
        ...
        try:
            ...
            self.periodic_login_enabled = self.config['Settings'].get('periodic_login_enabled', False)
